...
double CalculateDelay(int SF,int BW,int size,int CR){
    MSG("%d,%d,%d\n",SF,BW,CR);
    double Tsym, Tpreamble, Tpayload;
    uint32_t payloadSymbNb;
    Tsym = pow(2, SF) / BW;
    payloadSymbNb = 8 + (ceil((double)(8*size - 4*SF + 28 + 16) / (double)(4*SF)) * (CR + 4));
    Tpayload = payloadSymbNb * Tsym * 1000;
    return Tpayload;
}
...
int main(void)
{
    ...
    //chris 2023.1.2 open file
    timestamp_file = fopen("TimeStamp.csv","w+");
    tm_file = fopen("TM.csv","w+");
    if (timestamp_file == NULL) {
        MSG("ERROR: impossible to create file\n");
        exit(EXIT_FAILURE);
    }
    if (tm_file == NULL) {
        MSG("ERROR: impossible to create file\n");
    ...
    *(uint32_t *)(buff_up + 8) = net_mac_l;
    ...
    while (!exit_sig && !quit_sig) {
        ...
        for (i=0; i < nb_pkt; ++i) {
            ...
            //chris 2023.1.2 print orign recv data,write in the file
            //------------------------------------------------------
            MSG("%s\n",p->payload); 
                int SF,BW,CR;
                switch (p->bandwidth) {
            ...
            if(p->size == 16){
                ...
                int e = snprintf(databuff,17,"%s",p->payload);
                double toa = CalculateDelay(SF,BW,16,p->coderate);
                MSG("%lf\n",toa); //52240
                int ttt = (int) toa;
                e = fprintf(timestamp_file,"%u\n",p->count_us-ttt);
                if (e < 0) {
                    MSG("ERROR: impossible to write to log file\n");
                    exit(EXIT_FAILURE);
                }
                fflush(timestamp_file);
            }
            else if(p->size == 14){
                double toa = CalculateDelay(SF,BW,14,p->coderate);
                MSG("%lf\n",toa); //52240
                int ttt = (int) toa;

                int e = fprintf(tm_file,"%u\n",p->count_us-ttt);
                if (e < 0) {
                    MSG("ERROR: impossible to write to log file\n");
                    exit(EXIT_FAILURE);
                }
