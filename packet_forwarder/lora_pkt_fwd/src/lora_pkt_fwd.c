...
FILE * tx_file = NULL;
char tx_file_name[64];

//没什么用，用来记录发送的包的个数，本意是想让发送带上序号，接收方就能判断收到的包有没有掉，但是会造成发送长度变化，这样发送时间也变了，所以没什么用
int counter = 0;
//混合传输实验定义的，来控制发送次数，之后就不再发送，只接收
int flag;
...
/*------------------------------------------
@function name : open_tx_log
@param : void
@Description : open tx log, and name it like txlog_starttime
@author : william
@time : 2023.1.2
-------------------------------------------*/

void open_tx_log(void){
    int i;
    char iso_date[20];
    ...
    sprintf(tx_file_name, "txlog_%s.csv", iso_date);
    tx_file = fopen(tx_file_name, "a"); /* create log file, append if file already exist */
    if (tx_file == NULL) {
        MSG("ERROR: impossible to create log file %s\n", tx_file_name);
        exit(EXIT_FAILURE);
    }

    i = fprintf(tx_file, "\"us count\",\"size\",\"payload\",\"sec\",\"nsec\"\n");
    if (i < 0) {
...
@param : void
@Description : open rx log, and name it like pktlog_starttime
@author : william
@time : 2023.1.2
-------------------------------------------*/
...
void open_log(void) {
    ...
    sprintf(log_file_name, "pktlog_%s.csv", iso_date);
    ...
    i = fprintf(log_file, "\"mote addr\",\"freq\",\"us count\",\"size\",\"RSSI\",\"SNR\",\"payload\"\n");
    if (i < 0) {
        MSG("ERROR: impossible to write to log file %s\n", log_file_name);
        exit(EXIT_FAILURE);
    }
...
/*
@Description : 前面主要是配置网关的各类参数，然后会打开几个线程，主要关注接收线程thread_up和发送线程thread_jit（只控制发送队列发送，不控制把消息写入队列）
                然后会有一个无限循环，控制要发送什么消息。
@author : william
...
int main(void)
{
    ...
    /*-----------------------------------------------------
    @Description : 最重要的地方，在这里控制网关向节点发送的消息
    @author : william
    -----------------------------------------------------*/
    ...
    while (!exit_sig && !quit_sig) {
        ...
        /*------------------------------------------------------
        @Deseciption : txpkt就是网关要发送的消息的结构体，首先填完各种参数，然后压入队列即可
        @author : william
        ------------------------------------------------------*/
        ...
        txpkt.size = 5;
        sprintf(txpkt.payload,"Hello");
        
        /* Insert beacon packet in JiT queue */
        gettimeofday(&current_unix_time, NULL);
        get_concentrator_time(&current_concentrator_time, current_unix_time);
        jit_result = jit_enqueue(&jit_queue, &current_concentrator_time, &txpkt, JIT_PKT_TYPE_DOWNLINK_CLASS_C);
        ...
        MSG("INFO: [jit] queue contains %d pkt:\n", jit_queue.num_pkt);
        for (i=0; i<jit_queue.num_pkt; i++) {
            MSG(" - node[%d]: count_us=%u - type=%d\n",
                        i,
                        jit_queue.nodes[i].pkt.count_us,
                        jit_queue.nodes[i].pkt_type);
        }
    }
...
/*------------------------------------------------------
@Deseciption : 接收线程，还有利用socket上报到服务器的功能
@author : semtech
------------------------------------------------------*/
...
void thread_up(void) {
    ...
    *(uint32_t *)(buff_up + 8) = net_mac_l;
    ...
    @Description接收线程也是一个无限循环，本质就是一值抓取包，然后判断
    ...
    while (!exit_sig && !quit_sig) {
        ...
        for (i=0; i < nb_pkt; ++i) {
            ...
            //@Description : 下面很长一串都是处理packet成json，用于转发给服务器
            ...
            @Description : 在这里处理各种信息，写入log
            @author : william
            
            ------------------------------------------------------*/
            //------------------------------------------------------start
            
            fprintf(log_file, "%08x,", mote_addr);
            /* writing internal clock */
	        fprintf(log_file, "%.6lf,", ((double)p->freq_hz / 1e6));
            ...
            /* writing packet payload */
            char databuff[20];
            int e = snprintf(databuff,p->size+1,"%s",p->payload);
            fprintf(log_file, "%s%d", databuff,counter);
            printf("receive packet: %s%d\n",databuff,counter);
            ...
            if (pkt_index > -1) {
                ...
                if (jit_result == JIT_ERROR_OK) {
                    ...
                    if (result == LGW_HAL_ERROR) {
                        ...
                        @Description : 这里已经发送成功了，记录发送的包的各种信息
                        @author : william
                        
                        ---------------------------------------------*/

                        printf("lgw_send done: count_us=%u\n", pkt.count_us);
                        MSG_DEBUG(DEBUG_PKT_FWD, "lgw_send done: count_us=%u\n", pkt.count_us);
                        /* writing internal clock */
                        fprintf(tx_file, "%10u,", pkt.count_us);
                        /* writing payload size */
                        fprintf(tx_file, "%3u,", pkt.size);

                        /* writing packet payload */
                        char databuff[20];
                        int e = snprintf(databuff,pkt.size+1,"%s",pkt.payload);
                        fprintf(tx_file, "%s%d,", databuff,counter);
                        struct timespec ts;
                        clock_gettime(CLOCK_MONOTONIC,&ts);

                        fprintf(tx_file, "%ld,",ts.tv_sec);
                        fprintf(tx_file, "%ld",ts.tv_nsec);
                        /* end of log file line */
